...
uint32_t AudioPolicyManager::getDeviceForStrategy(routing_strategy strategy, bool fromCache)
{
    ...
    case STRATEGY_MEDIA: {
       ...
       }else if (mAvailableOutputDevices & AudioSystem::DEVICE_OUT_ANALOG_FM) {
           device |= AudioSystem::DEVICE_OUT_ANALOG_FM;
       }
                                                      ...
                                                      const char *device_address)
{
    ...
    if (AudioSystem::isOutputDevice(device)) {
        ...
        case AudioSystem::DEVICE_STATE_AVAILABLE:
            ...
            if(device == AudioSystem::DEVICE_OUT_ANALOG_FM){
                if(mAvailableOutputDevices & AudioSystem::DEVICE_OUT_FM){
                    LOGE("Digtl FM already on, analg FM switch in wrong state: Rejecting device connection");
                    return INVALID_OPERATION;
                }
                else
                    LOGW("FM started in Analog Mode");
            }

            if(device == AudioSystem::DEVICE_OUT_FM){
                if(mAvailableOutputDevices & AudioSystem::DEVICE_OUT_ANALOG_FM){
                    LOGE("Analog FM already on, Dgtl FM switch in wrong state: Rejecting device connection");
                    return INVALID_OPERATION;
                }
                else
                    LOGW("FM started in Digital Mode");
            }
        ...
        case AudioSystem::DEVICE_STATE_UNAVAILABLE: {
            if(device == AudioSystem::DEVICE_OUT_ANALOG_FM){
                LOGD("turning off AnalogFm device");
            }
                                             ...
                                             int session)
{
    ...
    if(stream == AudioSystem::SYSTEM){
       if(NewDevice == (AudioSystem::DEVICE_OUT_ANALOG_FM | AudioSystem::DEVICE_OUT_WIRED_HEADSET)){
            NewDevice = AudioSystem::DEVICE_OUT_ANALOG_FM_CODEC;
...
{
    LOGD("setOutputDevice() output %d device %x delayMs %d", output, device, delayMs);
    ...
    if (stream == AudioSystem::VOICE_CALL ||
        ...
        if (voiceVolume >= 0 && output == mHardwareOutput) {
            ...
            mLastVoiceVolume = voiceVolume;
                ...
                || (mAvailableOutputDevices & AudioSystem::DEVICE_OUT_ANALOG_FM)) ){
        float fmVolume = -1.0;
