...
status_t AudioHardware::doRouting(AudioStreamInMSM72xx *input)
{
    ...
    if (new_snd_device == -1) {
        ...
        if ((mTtyMode != TTY_OFF) && (mMode == AudioSystem::MODE_IN_CALL) &&
            ...
            new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
        } else if ((outputDevices & AudioSystem::DEVICE_OUT_ANALOG_FM) &&
                   ((outputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADSET) || (outputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADPHONE)) &&
                   (outputDevices & AudioSystem::DEVICE_OUT_SPEAKER)) {
            LOGI("Combo device:muting anlg FM and enabling Speaker\n");
            new_snd_device = SND_DEVICE_SPEAKER;
            new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
        } else if ((outputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADSET) &&
                   (outputDevices & AudioSystem::DEVICE_OUT_ANALOG_FM)) {
            LOGI("Enabling ANALOG FM device\n");
            new_snd_device = SND_DEVICE_FM_ANALOG_STEREO_HEADSET;
        } else if (outputDevices & AudioSystem::DEVICE_OUT_ANALOG_FM_CODEC){
            LOGI("Enabling ANALOG FM + codec device \n");
            new_snd_device = SND_DEVICE_FM_ANALOG_STEREO_HEADSET_CODEC;
            new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
        } else if ((outputDevices & AudioSystem::DEVICE_OUT_SPEAKER) &&
                   ((outputDevices & AudioSystem::DEVICE_OUT_FM) || (outputDevices & AudioSystem::DEVICE_OUT_ANALOG_FM))) {
            LOGI("Routing Dgtl FM to Speakerphone\n");
    ...
    if (((outputDevices & AudioSystem::DEVICE_OUT_FM) || enablefm) && (mFmFd == -1)) {
        enableFM();
    }
    if ((mFmFd != -1) && !(outputDevices & AudioSystem::DEVICE_OUT_FM) && !enablefm) {
        disableFM();
    }
