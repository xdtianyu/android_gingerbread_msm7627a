...
public class CameraHolder {
    ...
    private CameraHolder() {
        ...
        for (int i = 0; i < mNumberOfCameras; i++) {
            ...
            Log.e(TAG, "modes supported for camera id " + i + " = " + mInfo[i].mode);
        }
    }
    ...
    public boolean has3DSupport(int cameraId) {
        if((mInfo[cameraId].mode & CameraInfo.CAMERA_SUPPORT_MODE_3D) == CameraInfo.CAMERA_SUPPORT_MODE_3D)
                return true;
        return false;
    }

    public boolean hasZSLSupport(int cameraId) {
        if((mInfo[cameraId].mode & CameraInfo.CAMERA_SUPPORT_MODE_ZSL) == CameraInfo.CAMERA_SUPPORT_MODE_ZSL)
                return true;
        return false;
    }

    public synchronized android.hardware.Camera open(int cameraId, int mode)
            throws CameraHardwareException {
        Assert(mUsers == 0);
        if(cameraId > (mNumberOfCameras-1)) cameraId = 0;
        if (mCameraDevice != null && mCameraId != cameraId && mCameraMode != mode) {
        ...
        if (mCameraDevice == null) {
            try {
                ...
                mCameraDevice = android.hardware.Camera.open(cameraId, mode);
                mCameraId = cameraId;
                mCameraMode = mode;
    ...
    public synchronized android.hardware.Camera tryOpen(int cameraId) {
        try {
            return mUsers == 0 ? open(cameraId, 1) : null;
    ...
    public synchronized void keep() {
        ...
        if((mCameraMode & CameraInfo.CAMERA_SUPPORT_MODE_ZSL) != CameraInfo.CAMERA_SUPPORT_MODE_ZSL){
            //Always set this value in ZSL mode
