...
// Constructor
PVA_FF_EVRCSampleEntry::PVA_FF_EVRCSampleEntry(uint32 format)
...
// Destructor
PVA_FF_EVRCSampleEntry::~PVA_FF_EVRCSampleEntry()
{
    PV_MP4_FF_DELETE(NULL, PVA_FF_EVRCSpecificAtom, pEVRCSpecificAtom);
}


void
PVA_FF_EVRCSampleEntry::init()
{
    _reserved1[0] = 0;
    _reserved1[1] = 0;
    ...
    PV_MP4_FF_NEW(fp->auditCB, PVA_FF_EVRCSpecificAtom, (), pEVRCSpecificAtom);

    pEVRCSpecificAtom->setVendorcode();
    pEVRCSpecificAtom->setEncoderVersion();
    pEVRCSpecificAtom->setFramesPerSample(1);
}

// Rendering the PVA_FF_Atom in proper format (bitlengths, etc.) to an ostream
bool
PVA_FF_EVRCSampleEntry::renderToFileStream(MP4_AUTHOR_FF_FILE_IO_WRAP *fp)
{
    int32 rendered = 0;
    ...
    if (!pEVRCSpecificAtom->renderToFileStream(fp))
    {
        return false;
    }
    rendered += pEVRCSpecificAtom->getSize();

    return true;
}

void
PVA_FF_EVRCSampleEntry::recomputeSize()
{
    ...
    size += pEVRCSpecificAtom->getSize();
