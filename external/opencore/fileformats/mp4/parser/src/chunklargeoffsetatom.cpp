...
#define IMPLEMENT_ChunkLargeOffsetAtom

#include "chunklargeoffsetatom.h"
#include "atomutils.h"
#include "atomdefs.h"

// Ctor to create a new ChunkLargeOffsetAtom by reading in from an ifstream
ChunkLargeOffsetAtom::ChunkLargeOffsetAtom(MP4_FF_FILE *fp, uint32 size, uint32 type,
        ...
        : FullAtom(fp, size, type)
{
    ...
    if (_success)
    {
        ...
        uint64 dataSize = _size - (DEFAULT_FULL_ATOM_SIZE + 4);
            ...
            if (_entryCount > 0)
            {
                if (_parsingMode == 1)
                {
                    ...
                    if ((_entryCount > _stbl_buff_size))
                    {
                        ...
                        PV_MP4_FF_ARRAY_NEW(NULL, uint64, (_stbl_buff_size), _pchunkOffsets);
                ...
                _parsingMode = 0;
                PV_MP4_FF_ARRAY_NEW(NULL, uint64, (_entryCount), _pchunkOffsets);
...
// Destructor
ChunkLargeOffsetAtom::~ChunkLargeOffsetAtom()
{
    if (_pchunkOffsets != NULL)
    {
        // Cleanup vector
...
bool ChunkLargeOffsetAtom::ParseEntryUnit(uint32 sample_cnt)
{
...
int32
ChunkLargeOffsetAtom::getChunkOffsetAt(int32 index)
{
    if (_pchunkOffsets == NULL)
    {
        return PV_ERROR;
...
int32
ChunkLargeOffsetAtom::getChunkClosestToOffset(uint32 offSet, int32& index)
{
    index = -1;
