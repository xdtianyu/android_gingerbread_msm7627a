...
FreeChunkAvailableThreadSafeCB::FreeChunkAvailableThreadSafeCB(void* aObserver,
                                           uint32 aDepth,
                                           const char* aAOname,
                                           int32 aPriority)
:ThreadSafeCallbackAO(aObserver, aDepth, aAOname, aPriority)
...
FreeChunkAvailableThreadSafeCB::~FreeChunkAvailableThreadSafeCB( )
{

}

OsclAny* FreeChunkAvailableThreadSafeCB::Dequeue(OsclReturnCode &status)
{
    OsclAny *param;
    OsclProcStatus::eOsclProcError sema_status;
...
OsclReturnCode FreeChunkAvailableThreadSafeCB::ProcessEvent(OsclAny* EventData)
{
    if (iObserver != NULL)
    {
        PVMFOMXEncNode* ptr = (PVMFOMXEncNode*) iObserver;
...
void FreeChunkAvailableThreadSafeCB::Run()
{
    OsclAny *param;
    OsclReturnCode status = OsclSuccess;
