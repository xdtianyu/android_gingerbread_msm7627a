 ...
 * -------------------------------------------------------------------
 */
#ifndef PVMF_QCPFFPARSER_OUTPORT_H_INCLUDED
#define PVMF_QCPFFPARSER_OUTPORT_H_INCLUDED
...
typedef OsclMemAllocator PVMFQCPParserNodeAllocator;

#define PVMF_QCPPARSERNODE_LOGERROR(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iLogger,PVLOGMSG_ERR,m);
#define PVMF_QCPPARSERNODE_LOGWARNING(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iLogger,PVLOGMSG_WARNING,m);
#define PVMF_QCPPARSERNODE_LOGINFOHI(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG,iLogger,PVLOGMSG_INFO,m);
#define PVMF_QCPPARSERNODE_LOGINFOMED(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG,iLogger,PVLOGMSG_INFO,m);
#define PVMF_QCPPARSERNODE_LOGINFOLOW(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG,iLogger,PVLOGMSG_INFO,m);
#define PVMF_QCPPARSERNODE_LOGINFO(m) PVMF_QCPPARSERNODE_LOGINFOMED(m)
#define PVMF_QCPPARSERNODE_LOGSTACKTRACE(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG,iLogger,PVLOGMSG_STACK_TRACE,m);
#define PVMF_QCPPARSERNODE_LOGDATATRAFFIC(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iDataPathLogger,PVLOGMSG_INFO,m);
#define PVMF_QCPPARSERNODE_LOGCLOCK(m) PVLOGGER_LOGMSG(PVLOGMSG_INST_REL,iClockLogger,PVLOGMSG_INFO,m);
#define PVMF_QCPPARSERNODE_LOGBIN(iPortLogger, m) PVLOGGER_LOGBIN(PVLOGMSG_INST_LLDBG, iPortLogger, PVLOGMSG_ERR, m);


class PVMFQCPParserNodeLoggerDestructDealloc : public OsclDestructDealloc
{
    public:
...
class PVQCPFFNodeTrackPortInfo : public OsclMemPoolFixedChunkAllocatorObserver,
            public OsclMemPoolResizableAllocatorObserver
{
    public:

        PVQCPFFNodeTrackPortInfo()
        {
            iTrackId = -1;
            iPort = NULL;
        ...
        PVQCPFFNodeTrackPortInfo(const PVQCPFFNodeTrackPortInfo& aSrc) :
        ...
        virtual ~PVQCPFFNodeTrackPortInfo()
        {
        }
        ...
        void freechunkavailable(OsclAny* aContextData)
        {
            ...
            PVMF_QCPPARSERNODE_LOGINFO((0, "MimeType = %s, freeblockavailable", iTrackMimeType.get_cstr()));
            oQueueOutgoingMessages = true;
            if (iNode)
            {
        ...
        OSCL_HeapString<PVMFQCPParserNodeAllocator> iTrackMimeType;
        ...
        OSCL_HeapString<PVMFQCPParserNodeAllocator> iLogFile;
        bool oFormatSpecificInfoLogged;

        OsclSharedPtr<PVLoggerAppender> iBinAppenderPtr;
};

class PVMFQCPFFParserOutPort : public PvmfPortBaseImpl
            , public PvmiCapabilityAndConfig
{
    public:
         ...
         */
        PVMFQCPFFParserOutPort(int32 aTag
    ...
    private:
        ...
        PVMFQCPFFParserNode* iQCPParserNode;
};

#endif // PVMF_QCPFFPARSER_OUTPORT_H_INCLUDED
