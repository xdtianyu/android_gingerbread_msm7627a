...
void OmxDecTestPortReconfigTransitionTest_2::Run()
{
    switch (iState)
    {
        case StateUnLoaded:
        {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "OmxDecTestPortReconfigTransitionTest_2::Run() - OMX_MasterInit done"));

            Status = PrepareComponent();

            if (OMX_FALSE == Status)
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "OmxDecTestPortReconfigTransitionTest_2::Run() Error while loading component OUT"));
                iState = StateError;
        ...
        case StateStop:
        {
            ...
            if (OMX_ErrorNone != Err)
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "OmxDecTestPortReconfigTransitionTest_2::Run() - OMX_MasterDeinit Error"));
                iTestStatus = OMX_FALSE;
            }
