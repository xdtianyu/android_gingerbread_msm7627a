...
void OmxDecTestPauseResume::Run()
{
    switch (iState)
    {
        case StateUnLoaded:
        {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "OmxDecTestPauseResume::Run() - OMX_MasterInit done"));

            Status = PrepareComponent();

            if (OMX_FALSE == Status)
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "OmxDecTestPauseResume::Run() Error while loading component OUT"));
                iState = StateError;
        ...
        case StateStop:
        {
            ...
            if (OMX_ErrorNone != Err)
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "OmxDecTestPauseResume::Run() - OMX_MasterDeinit Error"));
                iTestStatus = OMX_FALSE;
            }
            ...
            else
            {
                if (OMX_FALSE == iTestStatus)
                {
                    ...
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_INFO,
                                    (0, "OmxDecTestPauseResume::Run() - %s : Fail", TestName));
                }
                else
                {
#ifdef PRINT_RESULT
...
#endif
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_INFO,
                                    (0, "OmxDecTestPauseResume::Run() - %s : Success {Output file not available}", TestName));
                }
            }
