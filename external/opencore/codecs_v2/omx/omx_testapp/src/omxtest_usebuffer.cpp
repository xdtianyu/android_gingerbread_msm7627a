...
void OmxDecTestUseBuffer::Run()
{
    switch (iState)
    {
        case StateUnLoaded:
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateUnLoaded IN"));
            ...
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "OmxDecTestUseBuffer::Run() - ERROR initCallbacks failed, OUT"));
                StopOnError();
                break;
            }
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "OmxDecTestUseBuffer::Run() - OMX_MasterInit done"));
            ...
            if (OMX_FALSE == Status)
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "OmxDecTestUseBuffer::Run() Error while loading component OUT"));
                iState = StateError;
                    ...
                    (NULL == ipThreadSafeHandlerFillBufferDone))
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR,
                                (0, "OmxDecTestUseBuffer::Run() - Error ThreadSafe Callback Handler initialization failed, OUT"));
            ...
            {
                iState = StateLoaded;
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateUnLoaded OUT, moving to next state"));
            }
        ...
        case StateLoaded:
        {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateLoaded IN"));
            ...
            ipUseInBuff = (OMX_U8**) oscl_malloc(sizeof(OMX_U8*) * iInBufferCount);
            CHECK_MEM(ipUseInBuff, "InputBuffer");
            ...
            ipUseOutBuff = (OMX_U8**) oscl_malloc(sizeof(OMX_U8*) * iOutBufferCount);
            CHECK_MEM(ipUseOutBuff, "OutputBuffer");
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                            (0, "OmxDecTestUseBuffer::Run() - Sent State Transition Command from Loaded->Idle"));

            iPendingCommands = 1;

            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                            (0, "OmxDecTestUseBuffer::Run() - Allocating %d input and %d output buffers", iInBufferCount, iOutBufferCount));

            //These calls are required because the control of in & out buffer should be with the testapp.
            for (ii = 0; ii < iInBufferCount; ii++)
            {
                ipUseInBuff[ii] = (OMX_U8*) oscl_malloc(iInBufferSize);
                CHECK_MEM(ipUseInBuff[ii], "InputBuffer");
                oscl_memset(ipUseInBuff[ii], 0, iInBufferSize);

                Err = OMX_UseBuffer(ipAppPriv->Handle, &ipInBuffer[ii], iInputPortIndex, NULL, iInBufferSize, ipUseInBuff[ii]);
                CHECK_ERROR(Err, "UseBuffer_Input");

                ipInputAvail[ii] = OMX_TRUE;
                ipInBuffer[ii]->nInputPortIndex = iInputPortIndex;

                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                (0, "OmxDecTestUseBuffer::Run() - Called UseBuffer for buffer index %d on port %d", ii, iInputPortIndex));
            ...
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR,
                                (0, "OmxDecTestUseBuffer::Run() - UseBuffer Error, StateLoaded OUT"));
                RunIfNotReady();
                break;
            }
            ...
            for (ii = 0; ii < iOutBufferCount; ii++)
            {
                ipUseOutBuff[ii] = (OMX_U8*) oscl_malloc(iOutBufferSize);
                CHECK_MEM(ipUseOutBuff[ii], "OutputBuffer");

                Err = OMX_UseBuffer(ipAppPriv->Handle, &ipOutBuffer[ii], iOutputPortIndex, NULL, iOutBufferSize, ipUseOutBuff[ii]);
                CHECK_ERROR(Err, "UseBuffer_Output");
                ...
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                (0, "OmxDecTestUseBuffer::Run() - Called UseBuffer for buffer index %d on port %d", ii, iOutputPortIndex));
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateLoaded OUT, Moving to next state"));
        ...
        case StateIdle:
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateIdle IN"));
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                            (0, "OmxDecTestUseBuffer::Run() - FillThisBuffer command called for initiating dynamic port reconfiguration"));
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                            (0, "OmxDecTestUseBuffer::Run() - Sent State Transition Command from Idle->Executing"));

            iPendingCommands = 1;

            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateIdle OUT"));
        }
        break;
        ...
        case StateDecodeHeader:
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "OmxDecTestUseBuffer::Run() - StateDecodeHeader IN, Sending configuration input buffers to the component to start dynamic port reconfiguration"));
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateDecodeHeader OUT"));
        }
        break;
        ...
        case StateDisablePort:
        {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateDisablePort IN"));
            ...
            if (!iDisableRun)
            {
                if (!iFlagDisablePort)
                {
                    ...
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                    (0, "OmxDecTestUseBuffer::Run() - Sent Command for OMX_CommandPortDisable on port %d as a part of dynamic port reconfiguration", iOutputPortIndex));
                ...
                else
                {
                    ...
                    {
                        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                        (0, "OmxDecTestUseBuffer::Run() - Not all the output buffers returned by component yet, wait for it"));
                        RunIfNotReady();
                        break;
                    }
                    ...
                    for (ii = 0; ii < iOutBufferCount; ii++)
                    {
                        if (ipOutBuffer[ii])
                        {
                            ...
                            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                            (0, "OmxDecTestUseBuffer::Run() - Called FreeBuffer for buffer index %d on port %d", ii, iOutputPortIndex));
                        }
                    }
                    ...
                    {
                        PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR,
                                        (0, "OmxDecTestUseBuffer::Run() - Error occured in this state, StateDisablePort OUT"));
                        RunIfNotReady();
                        break;
                    }
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateDisablePort OUT"));
        }
        break;
        ...
        case StateDynamicReconfig:
        {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateDynamicReconfig IN"));
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "OmxDecTestUseBuffer::Run() - GetParameter called for OMX_IndexParamPortDefinition on port %d", iParamPort.nPortIndex));
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "OmxDecTestUseBuffer::Run() new output buffers %d, size %d", iOutBufferCount, iOutBufferSize));
            ...
            CHECK_ERROR(Err, "SetParameter_IndexParamPortDefinition");

            Err = OMX_SendCommand(ipAppPriv->Handle, OMX_CommandPortEnable, iOutputPortIndex, NULL);
            CHECK_ERROR(Err, "SendCommand_PortEnable");

            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                            (0, "OmxDecTestUseBuffer::Run() - Sent Command for OMX_CommandPortEnable on port %d as a part of dynamic port reconfiguration", iOutputPortIndex));
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                            (0, "OmxDecTestUseBuffer::Run() - Allocating buffer again after port reconfigutauion has been complete"));
            ...
            for (ii = 0; ii < iOutBufferCount; ii++)
            {
                ...
                Err = OMX_UseBuffer(ipAppPriv->Handle, &ipOutBuffer[ii], iOutputPortIndex, NULL, iOutBufferSize, ipUseOutBuff[ii]);
                CHECK_ERROR(Err, "UseBuffer_Output_DynamicReconfig");
                ipOutReleased[ii] = OMX_TRUE;
                ...
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                (0, "OmxDecTestUseBuffer::Run() - UseBuffer called for buffer index %d on port %d", ii, iOutputPortIndex));
            }

            if (StateError == iState)
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR,
                                (0, "OmxDecTestUseBuffer::Run() - Error occured in this state, StateDynamicReconfig OUT"));
                break;
            }

            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateDynamicReconfig OUT"));
        }
        break;
        ...
        case StateExecuting:
        {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateExecuting IN"));
            ...
            while (MoreOutput)
            {
                ...
                if (Index != iOutBufferCount)
                {
                    ...
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                    (0, "OmxDecTestUseBuffer::Run() - FillThisBuffer command called for output buffer index %d", Index));
                }
                else
                {
            ...
            else if (OMX_FALSE == iEosFlagExecuting)
            {
                ...
                if (Index != iInBufferCount)
                {
                    ...
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                    (0, "OmxDecTestUseBuffer::Run() - Input buffer sent to the component with OMX_BUFFERFLAG_EOS flag set"));
                }
            }
            else
            {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateExecuting OUT"));
            RunIfNotReady();
        }
        break;
        ...
        case StateStopping:
        {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateStopping IN"));
            ...
            if (!iFlagStopping)
            {
                ...
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                (0, "OmxDecTestUseBuffer::Run() - Sent State Transition Command from Executing->Idle"));

                iPendingCommands = 1;
                iFlagStopping = OMX_TRUE;
            }

            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateStopping OUT"));
        }
        break;
        ...
        case StateCleanUp:
        {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateCleanUp IN"));
            ...
            if (!iFlagCleanUp)
            {
                ...
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                (0, "OmxDecTestUseBuffer::Run() - Sent State Transition Command from Idle->Loaded"));
                ...
                if (ipUseInBuff)
                {
                    for (ii = 0; ii < iInBufferCount; ii++)
                    {
                        if (ipUseInBuff[ii])
                        {
                            oscl_free(ipUseInBuff[ii]);
                            ipUseInBuff[ii] = NULL;
                        }
                    }

                    oscl_free(ipUseInBuff);
                    ipUseInBuff = NULL;
                }
                ...
                if (ipInBuffer)
                {
                    for (ii = 0; ii < iInBufferCount; ii++)
                    {
                        if (ipInBuffer[ii])
                        {
                            ...
                            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                            (0, "OmxDecTestUseBuffer::Run() - Called FreeBuffer for buffer index %d on port %d", ii, iInputPortIndex));
                        }
                    }
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateCleanUp OUT"));
        ...
        case StateStop:
        {
            OMX_U8 TestName[] = "NORMAL_SEQ_TEST_USEBUFF";
            OMX_ERRORTYPE Err = OMX_ErrorNone;

            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateStop IN"));
            ...
            if (ipAppPriv)
            {
                ...
                {
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                    (0, "OmxDecTestUseBuffer::Run() - Free the Component Handle"));

                    Err = OMX_MasterFreeHandle(ipAppPriv->Handle);
                    if (OMX_ErrorNone != Err)
                    {
                        PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "OmxDecTestUseBuffer::Run() - FreeHandle Error"));
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                            (0, "OmxDecTestUseBuffer::Run() - De-initialize the omx component"));

            Err = OMX_MasterDeinit();
            if (OMX_ErrorNone != Err)
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "OmxDecTestUseBuffer::Run() - OMX_MasterDeinit Error"));
                iTestStatus = OMX_FALSE;
            }
            ...
            else
            {
                if (OMX_FALSE == iTestStatus)
                {
                    ...
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_INFO,
                                    (0, "OmxDecTestUseBuffer::Run() - %s : Fail", TestName));
                }
                else
                {
#ifdef PRINT_RESULT
...
#endif
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_INFO,
                                    (0, "OmxDecTestUseBuffer::Run() - %s : Success {Output file not available}", TestName));
                }
            }

            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateStop OUT"));
        ...
        case StateError:
        {
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateError IN"));
            ...
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, (0, "OmxDecTestUseBuffer::Run() - StateError OUT"));
