    ...
    if (status == PVMFSuccess) {
        switch (command->code()) {
            case PlayerCommand::PLAYER_PREPARE:
                ...
                if (!mVideoOutputMIO && !((getFormatType() == PVMF_MIME_DATA_SOURCE_RTSP_URL) ||
                                          (getFormatType() == PVMF_MIME_DATA_SOURCE_MS_HTTP_STREAMING_URL) ||
                                          (getFormatType() == PVMF_MIME_DATA_SOURCE_SDP_FILE))) {
...
status_t doUsePVPlayer(const char *filename)
{
    ...
    if (status != OK) {
        ...
        if (amrParser.InitAMRFile(wFilename)) {
            LOGV("doUsePVPlayer: recognized .amr file");
            goto return_status;
        }
    }
