...
{
    printf("(");
    if (flags & SimpleNameService::IfConfigEntry::UP) {
        printf("UP ");
    }
    if (flags & SimpleNameService::IfConfigEntry::BROADCAST) {
        printf("BROADCAST ");
    }
    if (flags & SimpleNameService::IfConfigEntry::DEBUG) {
        printf("DEBUG ");
    }
    if (flags & SimpleNameService::IfConfigEntry::LOOPBACK) {
        printf("LOOPBACK ");
    }
    if (flags & SimpleNameService::IfConfigEntry::POINTOPOINT) {
        printf("POINTOPOINT ");
    }
    if (flags & SimpleNameService::IfConfigEntry::RUNNING) {
        printf("RUNNING ");
    }
    if (flags & SimpleNameService::IfConfigEntry::NOARP) {
        printf("NOARP ");
    }
    if (flags & SimpleNameService::IfConfigEntry::PROMISC) {
        printf("PROMISC ");
    }
    if (flags & SimpleNameService::IfConfigEntry::NOTRAILERS) {
        printf("NOTRAILERS ");
    }
    if (flags & SimpleNameService::IfConfigEntry::ALLMULTI) {
        printf("ALLMULTI ");
    }
    if (flags & SimpleNameService::IfConfigEntry::MASTER) {
        printf("MASTER ");
    }
    if (flags & SimpleNameService::IfConfigEntry::SLAVE) {
        printf("SLAVE ");
    }
    if (flags & SimpleNameService::IfConfigEntry::MULTICAST) {
        printf("MULTICAST ");
    }
    if (flags & SimpleNameService::IfConfigEntry::PORTSEL) {
        printf("PORTSEL ");
    }
    if (flags & SimpleNameService::IfConfigEntry::AUTOMEDIA) {
        printf("AUTOMEDIA ");
    }
    if (flags & SimpleNameService::IfConfigEntry::DYNAMIC) {
        printf("DYNAMIC ");
    }
    if (flags) printf("\b");
    printf(")");
...
int main(int argc, char** argv)
{
    ...
    //
    std::vector<SimpleNameService::IfConfigEntry> entries;
    ...
    for (uint32_t i = 0; i < entries.size(); ++i) {
        ...
        if (entries[i].m_flags & SimpleNameService::IfConfigEntry::UP) {
            if (entries[i].m_flags & SimpleNameService::IfConfigEntry::MULTICAST) {
                printf(", MTU = %d, address = %s", entries[i].m_mtu, entries[i].m_addr.c_str());
                if ((entries[i].m_flags & SimpleNameService::IfConfigEntry::LOOPBACK) == 0) {
