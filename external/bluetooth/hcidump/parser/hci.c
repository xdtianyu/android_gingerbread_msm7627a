...
#define GET_ARRAYSIZE(a) (sizeof(a)/sizeof(a[0]))
...
/* HCI events, ordered by event code */
...
#define EVENT_NUM (GET_ARRAYSIZE(event_str) - 1)

/* Link Control commands, ordered by OCF */
...
#define CMD_LINKCTL_NUM (GET_ARRAYSIZE(cmd_linkctl_str) - 1)

/* Link Policy commands, ordered by OCF */
...
#define CMD_LINKPOL_NUM (GET_ARRAYSIZE(cmd_linkpol_str) - 1)

/* Host Controller and Baseband commands, ordered by OCF */
...
#define CMD_HOSTCTL_NUM (GET_ARRAYSIZE(cmd_hostctl_str) - 1)
...
#define CMD_INFO_NUM (GET_ARRAYSIZE(cmd_info_str) - 1)
...
#define CMD_STATUS_NUM (GET_ARRAYSIZE(cmd_status_str) - 1)

static char *cmd_testing_str[] = {
	"Unknown",
	"Read Loopback Mode",
	"Write Loopback Mode",
...
#define CMD_TESTING_NUM (GET_ARRAYSIZE(cmd_testing_str) - 1)
...
#define ERROR_CODE_NUM (GET_ARRAYSIZE(error_code_str) - 1)

static inline void print_error_status(int level, struct frame *frm, char *status)
{
	p_indent(level, frm);
	printf("Error: %s\n", status);
}
...
static inline void create_physical_link_dump(int level, struct frame *frm)
{
	...
	for (i = 0; (i < cp->key_length) && (i < 32); i++)
...
static inline void write_remote_assoc_dump(int level, struct frame *frm)
{
	...
	p_indent(level, frm);
	printf("handle %d len_so_far %d assoc_len %d\n",
					cp->handle, btohs(cp->length_so_far),
					btohs(cp->assoc_length));
	len -= 5;
	for (i = 0; i < len; i++) {
		if (!(i%16)) p_indent(level, frm);
...
static inline void generic_status_response_dump(int level, struct frame *frm)
{
	...
	if (status > 0)
		print_error_status(level, frm, status2str(status));
...
static inline void generic_handle_response_dump(int level, struct frame *frm)
{
	uint8_t status = get_u8(frm);
	uint16_t handle = btohs(htons(get_u16(frm)));
...
static inline void read_link_policy_dump(int level, struct frame *frm)
{
	...
	if (rp->status > 0)
		print_error_status(level, frm, status2str(rp->status));
	else {
		str = hci_lptostr(policy);
		if (str) {
			p_indent(level, frm);
...
static inline void read_local_amp_info_dump(int level, struct frame *frm)
{
	...
	if (rp->status > 0)
		...
		printf("total bandwidth %d, max guaranteed bandwidth %d\n",
			   btohl(rp->total_bandwidth), btohl(rp->max_guaranteed_bandwidth));
		p_indent(level, frm);
		printf("min latency %d, max PDU size %d\n",
			   btohl(rp->min_latency), btohl(rp->max_pdu_size));
		p_indent(level, frm);
		printf("controller type 0x%2.2x, pal caps 0x%4.4x, max assoc length %d\n",
			   rp->controller_type, btohs(rp->pal_caps), btohs(rp->max_amp_assoc_length));
		p_indent(level, frm);
		printf("max flush timeout %d, best effort flush timeout %d\n",
			   btohl(rp->max_flush_timeout), btohl(rp->best_effort_flush_timeout));
	}
}
...
static inline void cmd_complete_dump(int level, struct frame *frm)
{
	...
	case OGF_LINK_CTL:
		...
		case OCF_EXIT_PERIODIC_INQUIRY:
		case OCF_READ_REMOTE_EXT_FEATURES:
			generic_status_response_dump(level, frm);
			return;
		case OCF_CREATE_CONN_CANCEL:
		case OCF_REMOTE_NAME_REQ_CANCEL:
		case OCF_PIN_CODE_REPLY:
	...
	case OGF_LINK_POLICY:
		...
		case OCF_WRITE_LINK_POLICY:
		case OCF_SNIFF_SUBRATING:
			generic_handle_response_dump(level, frm);
			return;
		case OCF_READ_DEFAULT_LINK_POLICY:
...
static inline void cmd_status_dump(int level, struct frame *frm)
{
	...
	if (evt->status > 0)
		print_error_status(level, frm, status2str(evt->status));
}
...
static inline void logical_link_complete_dump(int level, struct frame *frm)
{
	...
	printf("status 0x%2.2x log_handle %d handle %d tx_flow_id %d\n",
		   evt->status, btohs(evt->log_handle), evt->handle, evt->tx_flow_id);
...
static inline void event_dump(int level, struct frame *frm)
{
	...
	case EVT_INQUIRY_COMPLETE:
		generic_status_response_dump(level + 1, frm);
		break;
	...
	case EVT_CHANGE_CONN_LINK_KEY_COMPLETE:
		generic_handle_response_dump(level + 1, frm);
		break;
