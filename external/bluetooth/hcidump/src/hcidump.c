...
static void usage(void)
{
	...
	"  -M, --mode=mode            Default L2CAP mode\n"
	"  -E, --ecf                  Default is L2CAP Extended control fields\n"
...
int main(int argc, char *argv[])
{
	...
	while ((opt=getopt_long(argc, argv, "i:l:p:m:w:r:s:n:d:taxXRC:H:M:EO:P:D:A:BVYZN46h", main_options, NULL)) != -1) {
		switch(opt) {
		case 'i':
	...
	switch (mode) {
	case PARSE:
		init_parser(flags, filter, defpsm, defmode, defecf, defcompid, pppdump_fd, audio_fd);
