...
FMA2DPWriter::FMA2DPWriter()
    ...
    mAudioChannels(0),
    mSampleRate(0),
    mAudioFormat(android::AudioSystem::PCM_16_BIT),
    mAudioSource(android::AUDIO_SOURCE_FM_RX_A2DP),
    mBufferSize(0){
...
status_t FMA2DPWriter::readerthread() {
    ...
    int inChannel =
        (mAudioChannels == 2) ? AudioSystem::CHANNEL_IN_STEREO :
...
status_t FMA2DPWriter::writerthread(){
    ...
    //sizeof(int16_t) is frame size for PCM stream
    int outChannel = (mAudioChannels== 2) ? AudioSystem::CHANNEL_OUT_STEREO :
        AudioSystem::CHANNEL_OUT_MONO;

    prctl(PR_SET_NAME, (unsigned long)"FMA2DPWriterThread", 0, 0, 0);

    AudioTrack *audioTrack= new AudioTrack(
                android::AudioSystem::FM,
                mSampleRate,
                mAudioFormat,
                outChannel,
                framecount);
